...
export function ProfileOnboardingGate() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        const profile = await getLoggerContactProfile(user.uid)
        if (!active) return
        ...
        console.error("Unable to load contact profile:", error)
      } finally {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await saveLoggerContactProfile(user.uid, {
        phone: cleanedPhone,
      ...
      console.error("Unable to save contact profile:", error)
      ...
      >
        <DialogHeader className="items-center text-center">
          ...
          <DialogDescription className="max-w-sm text-balance leading-relaxed">
            Add your phone number so PawSattva can keep your pet logger contact details with your account.
          </DialogDescription>
        </DialogHeader>
