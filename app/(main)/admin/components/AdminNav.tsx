...
export function AdminNav({ activeTab, onTabChange }: AdminNavProps) {
  ...
  return (
    <>
      ...
      {/* ── Mobile top nav ── */}
      <div className="md:hidden flex items-center gap-1.5 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
        {NAV_GROUPS.flatMap((g) => g.items).map((item) => {
          ...
          return (
            <button
              ...
              onClick={() => onTabChange(item.primaryValue)}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap border shrink-0 transition-all duration-150",
                active
                  ? cn(item.activeBg, item.activeColor, item.activeBorder, "shadow-sm")
